...
``inception`` is a framework for submitting, tracking, and
post-processing parametric chombo-discharge studies on SLURM clusters.  A study
is declared as a Python ``Runs.py`` file and submitted via the ``inception``
...
* A *database* phase (fast / lightweight, e.g. a inception sweep) runs
